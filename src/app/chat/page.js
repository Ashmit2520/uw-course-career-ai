...
export default function ChatPage() {
  const [messages, setMessages] = useState([
    { from: "bot", text: "Hi! Tell me about your interests and what you want in a course." }
  ]);
  ...
  function handleSend(e) {
    ...
    setTimeout(() => {
      setMessages((msgs) => [
        ...msgs,
        { from: "bot", text: "Thanks for sharing! (In the future, I’ll recommend some courses here.)" }
      ]);
    }, 700);
  }
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-[80vh] px-2">
      <div className="w-full max-w-md md:max-w-lg bg-white shadow-2xl rounded-xl p-4 md:p-6 flex flex-col gap-4">
        ...
        <h2 className="text-2xl font-bold mb-2 text-center text-gray-900">Course Selection Chatbot</h2>
        ...
        >
          ...
          <button
            type="submit"
            className="bg-blue-600 text-white px-3 py-2 md:px-4 md:py-2 rounded-xl hover:bg-blue-700 transition text-sm md:text-base"
          >
            Send
          </button>
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18v4m0 0h3m-3 0H9m6-6v-5a3 3 0 10-6 0v5a3 3 0 006 0z" />
            </svg>
          </button>
        </form>
        {/* Enter-to-send hint */}
        <span className="text-xs text-gray-400 text-center mt-1">
          Hint: Press <span className="font-mono px-1 bg-gray-200 rounded">Enter</span> to send your message.
        </span>
        <span className="text-xs text-gray-400 text-center">Voice input coming soon</span>
      </div>
    </main>
  );
}
